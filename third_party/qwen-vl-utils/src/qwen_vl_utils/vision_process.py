...
def smart_nframes(
    ...
    if "nframes" in ele:
        ...
        # 确保 nframes 不超过 total_frames（向下取整到 FRAME_FACTOR 的倍数）
        if nframes > total_frames:
            max_possible_nframes = floor_by_factor(total_frames, FRAME_FACTOR)
            logger.warning(f"smart_nframes: requested nframes[{nframes}] > total_frames[{total_frames}], clamping to {max_possible_nframes}")
            nframes = max_possible_nframes
    else:
        fps = ele.get("fps", FPS)
